...
- `GET /api/analysis/{id}/state`: 查询当前状态（状态机）
- `GET /api/analysis/{id}/history`: 查询历史状态（状态机）
...
- 提供任务创建、更新、查询接口
- 支持 Redis 和文件两种存储方式
- 与 AsyncProgressTracker 独立，专为后端 API 服务

**核心方法**:
- `initialize(task_params)`: 初始化新任务
- `update_state(task_id, updates)`: 更新任务状态
- `get_current_state(task_id)`: 查询当前状态
- `get_history_states(task_id)`: 查询历史状态
...
class TaskStatus(BaseModel):
    ...
    progress: Dict[str, Any]       # 进度信息对象
    error: Optional[str]           # 错误信息
    result: Optional[Dict]         # 分析结果（完成时）
```
