...
| deep_think_llm | string | 深度思考模型名称：`qwen-max` / `gpt-4` / `gemini-pro`（用于复杂分析任务） |
| quick_think_llm | string | 快速思考模型名称：`qwen-plus` / `gpt-3.5-turbo`（用于简单快速任务） |
| cache_reuse_mode | string | 结果复用模式，见下表 |
| cache_reuse_sleep_min | number | 结果复用模拟延迟最小值（秒），默认 2.0 |
| cache_reuse_sleep_max | number | 结果复用模拟延迟最大值（秒），默认 10.0 |

**结果复用配置说明** (`cache_reuse_mode`):

当分析任务的参数（股票代码、分析日期、研究深度、分析师团队、市场类型）都相同时，系统可以从数据库缓存中读取历史分析结果，避免重复计算。

| 值 | 说明 |
|------|------|
| `false` 或 `""` | 禁用结果复用（默认） |
| `true` | 启用所有节点的结果复用 |
| `"market,fundamentals,news"` | 只对指定节点启用结果复用，支持逗号分隔的节点列表 |

**支持的节点名称**:
- `market` / `market_analyst` - 市场分析师
- `fundamentals` / `fundamentals_analyst` - 基本面分析师
- `news` / `news_analyst` - 新闻分析师
- `social` / `social_media_analyst` - 社交媒体分析师
- `bull` / `bull_researcher` - 看涨研究员
- `bear` / `bear_researcher` - 看跌研究员
- `research_manager` - 研究经理
- `trader` - 交易员
- `risky` / `risky_analyst` - 激进风险分析师
- `safe` / `safe_analyst` - 保守风险分析师
- `neutral` / `neutral_analyst` - 中性风险分析师
- `risk_manager` / `risk_judge` - 风险经理

**配置优先级**:
1. `extra_config` 中的 `cache_reuse_*` 参数（任务级别，优先级最高）
2. 环境变量 `CACHE_REUSE_*`（全局默认）
...
批量启动分析任务，多个股票代码使用相同的分析参数。

#### 请求

```http
POST /api/analysis/start/batch-same-params
Content-Type: application/json
```
...
```json
{
  "stock_symbols": ["603296", "300458", "600519"],
  "market_type": "A股",
  "analysis_date": "2025-12-02",
  "analysts": [
    "market_analyst",
  ...
  "extra_config": {
    ...
    "deep_think_llm": "qwen-max",
    "quick_think_llm": "qwen-plus",
    "cache_reuse_mode": "market,fundamentals,news",
    "cache_reuse_sleep_min": 1.0,
    "cache_reuse_sleep_max": 5.0
  }
}
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| stock_symbols | array | ✅ | 股票代码列表（如 ["603296", "300458"]） |
| market_type | string | ✅ | 市场类型：`美股` / `A股` / `港股` |
| analysis_date | string | ❌ | 分析日期，格式 YYYY-MM-DD，默认今天 |
| analysts | array | ✅ | 分析师列表 |
| research_depth | integer | ✅ | 研究深度，1-5，数字越大越深入 |
| include_sentiment | boolean | ❌ | 是否包含情绪分析，默认 true |
| include_risk_assessment | boolean | ❌ | 是否包含风险评估，默认 true |
| custom_prompt | string | ❌ | 自定义分析提示 |
...
|------|------|------|
| total | integer | 总任务数 |
| success_count | integer | 成功启动的任务数 |
| failed_count | integer | 启动失败的任务数 |
| tasks | array | 各任务的启动结果列表 |
...
| analysis_id | string | 分析任务唯一标识符（UUID），失败时为空 |
| status | string | 任务状态：`pending` / `failed` |
| message | string | 状态消息 |

#### 示例

```bash
curl -X POST http://localhost:8000/api/analysis/start/batch-same-params \
  -H "Content-Type: application/json" \
  -d '{
    "stock_symbols": ["603296", "300458"],
    "market_type": "A股",
    "analysts": ["market_analyst", "fundamental_analyst"],
    "research_depth": 3
  }'
...
批量启动分析任务，每个任务使用独立的参数配置。

#### 请求

```http
POST /api/analysis/start/batch
Content-Type: application/json
```
...
请求体为一个数组，每个元素的字段与 `/api/analysis/start` 接口相同：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| stock_symbol | string | ✅ | 股票代码（如 AAPL, 000001） |
| market_type | string | ✅ | 市场类型：`美股` / `A股` / `港股` |
| analysis_date | string | ❌ | 分析日期，格式 YYYY-MM-DD，默认今天 |
| analysts | array | ✅ | 分析师列表 |
| research_depth | integer | ✅ | 研究深度，1-5，数字越大越深入 |
...
```bash
curl -X POST http://localhost:8000/api/analysis/start/batch \
  -H "Content-Type: application/json" \
  -d '[
    {
      "stock_symbol": "603296",
...
```http
GET /api/analysis/management/count
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| task_id | string | ❌ | 任务ID筛选（支持部分匹配，不区分大小写） |
| analysis_date | string | ❌ | 分析日期筛选（精确匹配，格式：YYYY-MM-DD） |
| status | string | ❌ | 运行状态筛选（精确匹配，如：completed, running, pending, failed等） |
| stock_symbol | string | ❌ | 股票代码筛选（支持部分匹配，不区分大小写） |
| company_name | string | ❌ | 公司名称筛选（支持部分匹配，不区分大小写） |
...
```bash
# 获取所有任务总数
curl http://localhost:8000/api/analysis/management/count

# 按状态筛选
curl http://localhost:8000/api/analysis/management/count?status=completed

# 按股票代码筛选
curl http://localhost:8000/api/analysis/management/count?stock_symbol=603296

# 组合筛选
curl "http://localhost:8000/api/analysis/management/count?status=completed&stock_symbol=603296"
```
...
```http
GET /api/analysis/management/list
```
...
| page_size | integer | ❌ | 每页数量（1-100），默认10 |
| task_id | string | ❌ | 任务ID筛选（支持部分匹配，不区分大小写） |
| analysis_date | string | ❌ | 分析日期筛选（精确匹配，格式：YYYY-MM-DD） |
| status | string | ❌ | 运行状态筛选（精确匹配，如：completed, running, pending, failed等） |
| stock_symbol | string | ❌ | 股票代码筛选（支持部分匹配，不区分大小写） |
...
```json
{
  ...
  "data": [
    ...
    },
    {
      ...
      "status": "running",
      "created_at": "2025-12-02T15:00:00",
      "updated_at": "2025-12-02T15:05:00",
      "analysis_date": "2025-12-02",
      "stock_symbol": "300458",
      "company_name": "全志科技"
    }
...
```bash
# 获取第一页（默认10条）
curl http://localhost:8000/api/analysis/management/list

# 指定页码和每页数量
curl "http://localhost:8000/api/analysis/management/list?page=2&page_size=20"

# 按状态筛选
curl "http://localhost:8000/api/analysis/management/list?status=completed"

# 按股票代码筛选
curl "http://localhost:8000/api/analysis/management/list?stock_symbol=603296"

# 组合筛选和分页
curl "http://localhost:8000/api/analysis/management/list?page=1&page_size=10&status=completed&stock_symbol=603296"
```
...
根据任务ID获取任务的详细信息，包括current_step、history、props三个子键的数据。

#### 请求

```http
GET /api/analysis/management/{analysis_id}
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| current_step | object/null | 当前步骤信息 |
| history | array/null | 历史步骤列表 |
| props | object/null | 任务属性信息 |

**注意**: 每个字段值最多显示50个字符，多余的用省略号表示。
...
```json
{
  "detail": "未找到analysis_id为xxx的任务记录"
}
```
HTTP 状态码: 404
...
```bash
curl http://localhost:8000/api/analysis/management/a1b2c3d4-e5f6-7890-abcd-ef1234567890
```
