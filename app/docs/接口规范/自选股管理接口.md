...
| user_id | string | ❌ | 用户ID，如不提供默认为"guest" |
| stock_name | string | ❌ | 股票名称，如不提供将从股票字典自动查询 |
| market_type | string | ❌ | 市场类型（如：A股、港股、美股） |
| category | string | ❌ | 分类，默认"自选股"（注意：分类参与唯一性判断） |
...
**注意**：  
- 唯一性约束：同一用户ID、股票代码和分类的组合必须唯一。如果已存在相同的组合，接口返回 HTTP `400`，提示记录已存在。
- 如果未提供用户ID或用户ID为空，系统会自动使用"guest"作为默认值。
...
```http
GET /api/favorite-stocks?user_id=guest&category=自选股&limit=100&skip=0&sort_by=created_at&sort_order=desc
```
...
```http
GET /api/favorite-stocks/{stock_code}?user_id=guest&category=自选股
```
...
```http
PUT /api/favorite-stocks/{stock_code}?user_id=guest&category=自选股
Content-Type: application/json
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| user_id | string | ❌ | 用户ID，默认"guest" |
| category | string | ❌ | 分类（可选，用于定位要更新的记录） |
...
```http
DELETE /api/favorite-stocks/{stock_code}?user_id=guest&category=自选股
```
...
|------|------|------|------|
| user_id | string | ❌ | 用户ID，默认"guest" |
| category | string | ❌ | 分类（可选，用于定位要删除的记录） |
...
```http
GET /api/favorite-stocks/statistics/summary?user_id=guest
```
...
**状态**: ✅ 已完成  
**说明**: 批量创建自选股记录，支持一次添加多只股票到指定分类。

#### 请求

```http
POST /api/favorite-stocks/batch
Content-Type: application/json
```
...
```json
{
  "stock_codes": ["000001", "600036", "601166"],
  "user_id": "guest",
  "category": "银行板块",
  "tags": ["金融", "蓝筹"],
  "themes": ["银行"],
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| stock_codes | array[string] | ✅ | 股票代码列表（如：["000001", "600036"]） |
| user_id | string | ❌ | 用户ID，如不提供默认为"guest" |
| category | string | ❌ | 分类，默认"自选股" |
| tags | array[string] | ❌ | 标签列表，默认[] |
| themes | array[string] | ❌ | 概念板块（Theme）列表，默认[] |
| sectors | array[string] | ❌ | 行业板块（Sector）列表，默认[] |
| notes | string | ❌ | 备注，默认"无" |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| success | boolean | 是否全部成功（所有股票都成功） |
| data | object | 批量创建结果详情 |
| data.total | integer | 总股票数量 |
| data.success_count | integer | 成功创建的股票数量 |
| data.failed_count | integer | 创建失败的股票数量 |
| data.success_list | array[string] | 成功创建的股票代码列表 |
| data.failed_list | array[object] | 创建失败的股票列表，每个元素包含 stock_code 和 error |
| message | string | 描述信息 |

**注意**：  
- 批量创建时，如果某只股票已存在（同一用户、股票代码和分类的组合），该股票会创建失败，但不会影响其他股票的创建。  
- 如果所有股票都创建成功，`success` 为 `true`；如果至少有一只股票失败，`success` 为 `false`。  
- 失败的股票信息会记录在 `failed_list` 中，便于排查问题。
...
- 同一用户ID、股票代码和分类的组合必须唯一（复合唯一索引：`user_id + stock_code + category`）
- 这意味着同一用户可以多次添加同一只股票，只要分类不同即可
...
```bash
curl "http://localhost:8000/api/favorite-stocks?category=自选股&limit=10"
```
