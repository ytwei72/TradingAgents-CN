...
| 字段 | 类型 | 说明 |
|------|------|------|
| success | boolean | 是否全部更新成功（所有板块都成功） |
| data | object | 更新结果详情 |
| message | string | 描述信息 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| success | array[string] | 成功更新的板块名称列表 |
| failed | object | 更新失败的板块字典，键为板块名称，值为错误信息 |
| total | integer | 总板块数量 |
| success_count | integer | 成功更新的板块数量 |
| failed_count | integer | 更新失败的板块数量 |
| updated | boolean | 是否全部更新成功（failed_count 为 0） |
...
- `failed` 字段包含所有失败的板块及其错误信息，便于排查问题。
...
**状态**: ✅ 已完成  
**说明**: 更新指定的概念板块列表（从 akshare 获取最新数据并保存到数据库）。

#### 请求

```http
POST /api/stock-data/sectors/concept/update
Content-Type: application/json
```
...
```json
{
  ...
  "message": "概念板块更新完成，成功 2 个，失败 1 个"
}
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| success | boolean | 是否全部更新成功（所有指定板块都成功） |
| data | object | 更新结果详情 |
| message | string | 描述信息 |
...
**注意**：  
- 更新过程可能需要较长时间，因为需要从 akshare 获取数据并逐个保存。  
- 如果板块名称列表为空，返回 HTTP `400`。  
- 如果板块数据服务不可用或 akshare 库未安装，返回 HTTP `500`。  
- 更新过程中如果某个板块更新失败，会继续更新其他板块。
...
**状态**: ✅ 已完成  
**说明**: 更新指定的行业板块列表（从 akshare 获取最新数据并保存到数据库）。

#### 请求

```http
POST /api/stock-data/sectors/industry/update
Content-Type: application/json
```
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| industry_names | array[string] | ✅ | 行业板块名称列表 |
...
```json
{
  ...
  "message": "行业板块更新完成，成功 3 个，失败 0 个"
}
```
