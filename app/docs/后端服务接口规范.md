...
获取系统配置（持久化覆盖项）。系统配置包括 LLM 提供商、模型设置、研究深度、市场类型、功能开关、数据库配置等。
...
|------|------|------|
| llm_provider | string | LLM 提供商：`dashscope` / `openai` / `google` |
| deep_think_llm | string | 深度思考模型名称 |
| quick_think_llm | string | 快速思考模型名称 |
| research_depth_default | integer | 默认研究深度（1-5） |
| market_type_default | string | 默认市场类型：`美股` / `A股` / `港股` |
| memory_enabled | boolean | 是否启用记忆功能 |
| online_tools | boolean | 是否启用在线工具 |
| online_news | boolean | 是否启用在线新闻 |
| realtime_data | boolean | 是否启用实时数据 |
| max_recur_limit | integer | 最大递归限制（≥1） |
| backend_url | string | LLM 后端地址 |
| custom_openai_base_url | string/null | 自定义 OpenAI Base URL |
| data_dir | string | 数据目录路径 |
| results_dir | string | 结果目录路径 |
| data_cache_dir | string | 数据缓存目录路径 |
| db | object | 数据库配置对象 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| mongo_host | string | MongoDB 主机地址 |
| mongo_port | integer | MongoDB 端口号 |
| mongo_username | string/null | MongoDB 用户名 |
| mongo_password | string/null | MongoDB 密码 |
| mongo_database | string | MongoDB 数据库名称 |
| mongo_auth_source | string | 认证数据库 |
| mongo_max_connections | integer | 最大连接数 |
| mongo_min_connections | integer | 最小连接数 |
| mongo_connect_timeout_ms | integer | 连接超时（毫秒） |
| mongo_socket_timeout_ms | integer | Socket 超时（毫秒） |
| mongo_server_selection_timeout_ms | integer | Server 选择超时（毫秒） |
| mongo_uri | string | MongoDB 连接 URI |
| mongo_db | string | 数据库名称 |
...
更新系统配置并持久化。支持部分更新，只传递需要修改的字段即可。配置会与现有配置进行深度合并。
...
```json
{
  ...
  "db": {
    "mongo": {
      ...
      "mongo_port": 27017,
      "mongo_database": "tradingagents_prod"
    }
  }
}
```
...
所有参数均为可选，只传递需要更新的字段即可。参数说明与 [5.1 获取系统配置](#51-获取系统配置) 中的响应字段说明一致。

**注意事项**:
- 支持嵌套对象的深度合并，例如只更新 `db.mongo.mongo_host` 不会影响其他 MongoDB 配置
- `research_depth_default` 的取值范围为 1-5
- `max_recur_limit` 必须 ≥ 1
- 传递 `null` 值会保留现有配置，不会清空字段
...
响应格式与 [5.1 获取系统配置](#51-获取系统配置) 相同，返回更新后的完整配置。
  ...
  "detail": [
    {
      ...
      "loc": ["body", "research_depth_default"],
