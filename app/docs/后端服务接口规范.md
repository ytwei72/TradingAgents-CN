...
获取系统配置。系统配置包括四种类型：`models`（模型配置）、`pricing`（定价配置）、`usage`（使用记录）、`settings`（设置配置）。可以通过查询参数指定要获取的配置类型。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| config_types | string | ❌ | 逗号分隔的配置类型列表，可选值：`models`、`pricing`、`usage`、`settings`。如果不指定，默认只返回 `settings` |
...
- **usage**: 使用记录列表，每个元素包含 `timestamp`、`provider`、`model_name`、`input_tokens`、`output_tokens`、`cost`、`session_id`、`analysis_type` 等字段
...
```json
{
  "detail": "无效的配置类型: ['invalid_type']。有效类型: ['models', 'pricing', 'usage', 'settings']"
}
```
HTTP 状态码: 400
...
**获取所有配置**:
```bash
curl "http://localhost:8000/api/config/system?config_types=models,pricing,usage,settings"
```
...
更新系统配置并持久化。请求体是一个 JSON 对象，可以包含 `models`、`pricing`、`usage`、`settings` 中的任意几个键。根据请求中包含的键来更新对应的配置。
...
- **usage**: `List[Dict]` - 使用记录列表，会完全替换现有使用记录
...
| usage | List[Dict] | ❌ | 使用记录列表，每个元素包含 `timestamp`、`provider`、`model_name`、`input_tokens`、`output_tokens`、`cost`、`session_id`、`analysis_type` 等字段 |
...
```json
{
  "detail": "请求中必须包含至少一个配置类型。有效类型: ['models', 'pricing', 'usage', 'settings']"
}
```
HTTP 状态码: 400
